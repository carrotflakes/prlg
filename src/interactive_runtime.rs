...
use crate::{World, bindings::Bindings, data::Data};
...
impl<'a> InteractiveRuntime<'a> {
    pub fn new(world: &'a World) -> Self { Self { world } }
    ...
    pub fn run(&mut self, mut goals: Vec<Data>) {
        ...
        let rest_goals = binder.instances(&goals);
        binder.alloc(max_var);
        let mut candidates: Vec<(Vec<Data>, Vec<Data>)> = self.world.rules.iter().filter_map(|rule| {
            ...
            if binder.unify(left.clone(), right) {
                ...
                let body = binder.instances(&rule.body);
